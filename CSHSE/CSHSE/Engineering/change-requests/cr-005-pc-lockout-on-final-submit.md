...
Code-verified during the 2026-05-31 sweep. **Working:** `submitSelfStudy` flips `status='submitted'` + `readerLock.isLocked=true` (`server/src/controllers/submissionController.ts:1609-1620`); `submissionLockout` middleware returns 403 for a locked PC and is mounted on narrative/intro save, review approve/discard/finish/apply, matrix-state, submitStandard, revert, revalidate, markStandardComplete, and N/A set/clear (`server/src/middleware/submissionLockout.ts:52-75`, `routes/submissions.ts:81-222`); the client shows a read-only/print banner (`SelfStudyEditor.tsx:2267-2280`). **Two gaps remain:** (1) **evidence mutation routes are NOT locked** — `routes/evidence.ts:112-166` mount upload/url/update/delete/link/unlink with no `submissionLockout` and `evidenceController` has zero lock checks, so a locked PC can still mutate supporting evidence (see [[sprint-plan-2026-05-31]] §2 BUG-B); (2) the client editor inputs gate on `isReadOnly = !isProgramCoordinator` (`SelfStudyEditor.tsx:485`), which doesn't include the lock, so post-submit the fields stay editable (banner only; saves fail at the server). Remaining work: lock evidence routes + make `isReadOnly`/`isEditingDisabled` incorporate `isSubmissionLocked`. Scheduled Sprint 10 in [[sprint-plan-2026-05-31]].
