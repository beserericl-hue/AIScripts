...
| [[cr-042-memberclick-sso-api-entry-point]] | **Public** SSO API — versioned (`/api/v1/`), OpenAPI 3.1 spec, per-key tiered rate limits + dashboards, sandbox env, status page; MemberClick is consumer #1 (with a **non-programmer admin walkthrough**: integration-package generator + relay endpoint with 4-defense validation — Referer + IP + email-domain + timestamp); also the password-less E2E auth path (kills plaintext passwords in CR-034 fixtures) | proposed | User direction 2026-05-23 (MemberClick + future partners + non-programmer admin steps + E2E password elimination) |
