...
- **Review-surface change-kind persistence (FIXED, 2026-06-05).** While E2E-verifying persistence, spec `43_review_autosave` (Review-rail narrative→evidence kind flip surviving a reload) was found failing on the live deploy. CR-059 didn't change that path logically (`git diff` confirms it), but its larger bundle shifted load timing enough to tip a latent race from passing to failing. Two compounding root causes, both now fixed:
  1. **Late-load clobber** — the cards render instantly from the localStorage seed, so the PC can flip a kind WHILE `loadPersistedReviewState`'s GET is in flight; applying the server copy then reverted the unsaved edit. Fix: a minimal, object-identity, per-fetch guard — `loadPersistedReviewState` keeps local `buckets` only when their identity changed during *this* GET (`aiImportStore.ts`). Byte-identical to the original for every other field/flow (an earlier `dirty`-based guard wrongly kept stale local state on seeded reloads and regressed 39/40/41/44/45/46; the persisted `dirty` flag is conflated with the wizard's needs, so it can't gate this).
  2. **Mount-vs-edit write race** — the seed carries `dirty:true`, so a mount-time debounced autosave POSTs the pre-edit buckets; the user's edit fires a second autosave; on the network the stale POST could land last and persist the pre-edit state. Fix: `saveReviewStateToServer` now serializes every save through one promise queue, each reading `get()` at run time, so the server always ends with the latest buckets.
  An immediate-save in `handleChangeKind` was tried and **removed** — it cleared `dirty` before the 1.2s debounce fired and left the "Saving…" chip stuck. The clean fix is the two changes above; the existing debounced autosave handles the write. Verified: a diagnostic confirmed the server holds the item in `evidenceText` after the flip AND after a reload; spec 43 passes (3/3 sequential, 15/16 in the full parallel suite; the lone miss was a 15s UI-render timeout under artificial 4×-concurrent load on the dev instance, not a persistence loss).
- **Reader + lead-reader persistence (verified).** New live specs `52` (reader 0–3 score → `Score`, persists across reload + re-score upsert) and `53` (lead reader consolidated final score → `LeadFinalScore`, persists + clears on DELETE) pass on the live deploy. These roles write directly via react-query → REST (no localStorage seed/clobber pattern), so they were never vulnerable to the PC-surface race; the 75-test reader/lead-reader integration suite stays green.
