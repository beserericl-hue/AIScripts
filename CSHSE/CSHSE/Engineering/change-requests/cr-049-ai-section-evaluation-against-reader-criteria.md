...
**Phase 3 shipped 2026-05-29 — editor "AI Review" surface.**
- Server: `GET /:id/standards/:std/specs/:spec/evaluation` (latest `ValidationResult` verdict/rationale/criteriaCoverage) + `POST .../evaluate` ("Run AI Review" — runs `validateSection` for one spec, owner-PC/admin, persists). Wired in `routes/submissions.ts`.
...
**Phase 6 shipped 2026-05-30 — Reader override UI (Sprint 3 / S3.4).**
- `client/src/features/reader/ReaderOverrideControl.tsx` — pass / needs_improvement / fail radio + optional note + Save button; mounted in `ReaderSpecRow` next to the AI verdict so the reader sees both at once. Calls `POST /api/submissions/:id/standards/:std/specs/:spec/override`, which (Phase 4b) stamps the verdict + ingests `section_eval_override` into Qdrant. The next evaluation (Phase 5 `hints_fn`) reads it back as a soft hint. The loop is now fully closed end-to-end through a real UI.
- 7 unit tests on `ReaderOverrideView` (verdict choices, draft vs current, disabled-when-same, saving disables everything, overridden-chip).
- CR-049 status → **shipped**. Future polish (showing per-link `linkVerdict` chips inline, batched-override flows) is out of scope.
...
- Test pyramid (real CSHSE spec data via `load_specifications` makes all three layers possible):
  - **Unit** (`tests/test_section_eval.py`, 19) — html→text, evaluable classification (org-chart/image → review), scrape fail-soft, response parse + verdict defaulting, single/many-spec evaluation with an injected fake client, unevaluable-link surfaced (not failed), no-API-key degradation.
  - **System/integration** (`tests/test_section_eval_endpoint.py`, 6) — the HMAC-verified FastAPI endpoint driven with **real CSHSE spec criteria**: missing/bad HMAC → 401, malformed body → 422, real specs → one verdict row each (codes match), graceful degradation (no key → 200, not 500), and a patched-LLM `pass` verdict through the full endpoint path.
  - **E2E (opt-in)** — `test_e2e_real_llm_evaluates_a_real_spec` runs the real evaluator against a real Anthropic LLM with a real spec + narrative; skipped unless `RUN_LLM_E2E=1` + `ANTHROPIC_API_KEY`. (The full server→ai-service→LLM E2E lands with Phase 2's server wiring.)
  - Full ai-service suite: 398 passed / 5 skipped.
