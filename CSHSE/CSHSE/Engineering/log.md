...
## [2026-05-29] update | CR-049 OQ resolved + CR-050 (empty specs must not block submission)

Two follow-ups from the user:

1. **CR-049 OQ3 resolved:** Final Submit **auto-runs a full evaluation** across every applicable spec — re-evaluating even sections already run individually — and that on-submit pass is the authoritative **reader-report seed**. Excluded/N-A specs are skipped. Updated [[cr-049-ai-section-evaluation-against-reader-criteria]] (Decision/Acceptance/OQ3 + revision_history).

2. **New [[cr-050-intentionally-omitted-specs-do-not-block-submission]] (P1, Sprint 2A):** confirmed the live blocker — both submit gates require EVERY spec `validationStatus==='pass'` (server `submitSelfStudy` ~`submissionController.ts:1051`; client `isSelfStudyReadyForSubmit` `SelfStudyEditor.tsx:2108`), and `Submission.standardsStatus` has no `not_applicable`/`excluded` value (`models/Submission.ts:196-204`). So an intentionally-empty spec hard-blocks Final Submit with no override. CR-050 adds an explicit PC-set "Not applicable" state (with reason); readiness becomes pass-OR-excluded; excluded specs render N/A (not fail) in the CR-008 popup, the CR-049 eval, and the reader report (`Review.not_applicable` already exists, `models/Review.ts:93`). Naive "don't block on empty" rejected — it would leak accidental gaps; intent must be explicit. Slotted as Sprint 2A S2A.3; index row added.

## [2026-05-29] update | CR-049 — web-scrape behavior + reader override→learning loop

Two more CR-049 refinements from the user:
- **Web-link scraping (OQ1 resolved):** fetch each evidence link, **strip HTML to raw text**, evaluate the text against the standard's evidence criteria (no headless render). Links that aren't text-evaluable (org chart, image, diagram, login-walled) return `linkVerdict: 'needs_human_review'` and are surfaced for the reader to open directly — **never auto-failed**, so a non-text evidence link can't sink an otherwise-good section.
...
## [2026-05-29] update | CR-049 Phase 1 — ai-service section evaluator shipped

Built the cshse-ai core of the section evaluator (`ai-service/app/section_eval/`): `scrape.py` (html→text + nav/footer strip; image/diagram/login-walled/short pages → `needs_human_review`, never auto-failed; httpx fetch fail-soft) + `evaluate.py` (`evaluate_section` → per-spec verdict pass/needs_improvement/fail + rationale + criteriaCoverage + improvementSuggestions + sourcesUsed; injectable Anthropic, strict-then-loose JSON parse, fail-soft). New HMAC-verified `POST /ai/section/evaluate` in `app/main.py`. Criteria are passed in by the caller (server resolves from the rubric) so the evaluator is decoupled from rubric storage. 19 new unit tests; full ai-service suite 392 passed / 4 skipped. [[cr-049-ai-section-evaluation-against-reader-criteria]] → in-progress (Phase 1). Remaining: Phase 2 server wiring + n8n retire, Phase 3 editor surface, Phase 4 on-submit auto-eval + reader override learning loop.

## [2026-05-29] update | CR-049 Phase 1 — system/integration + E2E tests added

Filled the test pyramid for the section evaluator (real CSHSE spec data makes all three layers possible). Added `ai-service/tests/test_section_eval_endpoint.py` (6): HMAC-verified `/ai/section/evaluate` endpoint driven with real `load_specifications` criteria — missing/bad HMAC → 401, malformed body → 422, real specs → one verdict row each (codes match), no-key graceful degradation (200 not 500), and a patched-LLM `pass` verdict through the full endpoint path. Plus an opt-in real-LLM E2E (`test_e2e_real_llm_evaluates_a_real_spec`, skipped unless `RUN_LLM_E2E=1` + `ANTHROPIC_API_KEY`). Layers now: 19 unit + 6 integration + 1 opt-in E2E; full ai-service suite 398 passed / 5 skipped. Full server→ai-service→LLM E2E lands with Phase 2.

## [2026-05-29] update | CR-049 Phase 2 — server wiring (replaces broken validateSection + n8n)

Wired the server side of the section evaluator. `cshseAiClient.evaluateSection` (HMAC POST to `/ai/section/evaluate`). New `ValidationService.validateSection` — the method `submitStandard`/`revalidateFailed` were calling but that DIDN'T EXIST (R.1 finding): resolves the spec's rubric criteria from `getAllStandards()`, calls cshse-ai, maps the 3-level verdict to the binary submit gate (pass→pass else fail) while keeping verdict+rationale+suggestions, persists a `ValidationResult`, fail-softs to `fail` when cshse-ai is unreachable so Final Submit stays usable. `ValidationResult` gains `needs_improvement` + `verdict`/`rationale`/`criteriaCoverage`. **n8n validation superseded for the submit path** (legacy `triggerValidation` + webhook callback remain but are off the submit path — deletable in a follow-on). This is the fix that lets a spec reach `pass` — the last code blocker for CR-006. Tests: `validate-section.test.ts` (4, cshse-ai spied) + updated submission-lockout characterization. Full server suite green. CR-049 Phase 2 of 4 done; Phase 3 (editor surface) + Phase 4 (on-submit auto-eval + reader-override learning) remain.

## [2026-05-29] update | CR-049 — Phases 1+2 shipped; Phase 3/4 scoped

CR-049 stands at Phases 1 (ai-service evaluator) + 2 (server wiring) shipped & deployed — the section evaluator works end-to-end at the API layer and the broken n8n `validateSection` is replaced (the last code blocker for CR-006: specs can now reach `pass`). Remaining scoped: **Phase 3** editor surface (per-section "Evaluate" + AI-review panel; needs a latest-ValidationResult read endpoint + single-spec evaluate trigger + UI). **Phase 4a** Final-Submit auto-eval to seed the reader report — deliberately deferred to a **background job** (evaluating ~99 specs through the LLM synchronously in `submitSelfStudy` would time out; fire-and-forget in a Railway container is unreliable). **Phase 4b** reader override→learning loop — UI is blocked on the Sprint 3 reader client; CR-049 owns the `/ai/corrections/ingest` ingest wiring. Updated [[cr-049-ai-section-evaluation-against-reader-criteria]].

## [2026-05-29] update | CR-049 Phase 3 — editor "AI Review" surface shipped

PCs now see an **AI Review** panel under each spec's narrative editor: verdict (pass / needs improvement / fail) + rationale + improvement suggestions + criteria coverage, with a **Run AI Review** button (readers see the verdict read-only). Server: `GET /:id/standards/:std/specs/:spec/evaluation` (latest verdict) + `POST .../evaluate` (run now, owner-PC/admin). Client: `SpecAIReview.tsx` (pure `SpecAIReviewView` + query/mutation container) mounted in `SelfStudyEditor`. Hardening: the submitStandard characterization test now mocks the AI client (no real network → fast, no teardown rejection) and `cshseAiClient.postSigned` got a 60s AbortController timeout so a stuck ai-service can't hang the submit path. Tests: client `SpecAIReview.test.tsx` (7) + server `spec-evaluation-endpoint.test.ts` (3); full server 282/282, full client 198/198. (Server suite exits 1 on an intermittent pre-existing GridFS-cleanup teardown race — not a test failure.) CR-049 Phase 3 of 4 done; Phase 4a (bg auto-eval) + 4b (reader-override ingest) remain.

## [2026-05-29] update | CR-049 Phase 4a — Final-Submit auto-eval (reader-report seed)

`ValidationService.runReaderReportSeed(submissionId)` re-evaluates every spec with narrative content (one `validateSection` per spec) and persists a fresh `ValidationResult` — the authoritative reader-report seed produced at submission, even for sections already evaluated per-standard. `submitSelfStudy` fires it **detached** after save/audit so the response returns immediately; best-effort (failure never blocks submit). Detached background work is reliable here — cshse-server is a persistent Express container (SSE/webhooks), not serverless. Test: `validate-section.test.ts` runReaderReportSeed (evaluates only content-bearing specs + persists each); submitSelfStudy still 13/13. CR-049 Phase 4a done; only Phase 4b (reader override → `/ai/corrections/ingest` learning ingest; UI on the Sprint 3 reader client) remains.

## [2026-05-29] update | CR-049 Phase 4b — reader override → learning ingest

Server: `POST /:id/standards/:std/specs/:spec/override` (reader/lead/admin) — the reader's verdict becomes the authoritative verdict on the latest `ValidationResult` (`readerOverridden` + note) AND is fed to the institution's RAG via `cshseAiClient.ingestCorrection` (`correctionType: 'section_eval_override'`, best-effort). ai-service: `evaluate_section` gained an injectable `hints_fn(std, spec)` that surfaces prior override hints into the per-spec prompt (learning read-side), fail-soft. Tests: server `section-eval-override.test.ts` (3) + ai-service hint tests (+2); ai-service 400/5-skip; server CR-049 files 24. Remaining is Sprint-3-coupled only: the reader-override **UI** (reader client) + activating the endpoint's `hints_fn` against live Qdrant (one step, deferred until overrides exist). CR-049 substantively complete (Phases 1-4 server/ai/editor all shipped); the reader-override button lands with the Sprint 3 reader client.

## [2026-05-29] update | Sprint 2A complete — CR-005/CR-006/CR-050 shipped, CR-008 Phase 1 shipped

S2A.3 / **CR-050**: per-spec `excluded/excludedReason/excludedAt/excludedBy` on `standardsStatus` (`models/Submission.ts`); `submitSelfStudy` predicate now pass-OR-excluded; `markSpecNotApplicable` + `clearSpecNotApplicable` controllers + `/standards/:std/specs/:spec/not-applicable` routes; `runReaderReportSeed` skips excluded specs; `WorkflowSummary` denominator drops them; client `isSelfStudyReadyForSubmit` predicate matches server; new `SpecNotApplicable` view in editor. 8 server + 5 client tests passing.

S2A.1: audit on every reader-lock transition — `submission.reader_lock` on lock, `submission.reader_unlock` on release, `submission.send_back` (with the reader's reason), `submission.sent_back_cleared` on PC re-submit; `reader.assigned` per reader on assignment (so the timeline shows fan-out, not a blob). `AuditAction` union widened to include `submission.sent_back_cleared`, `submission.spec_marked_na`, `submission.spec_na_cleared`. 5 audit-transition integration tests passing.

S2A.4 / **CR-005**: new admin-only `POST /api/submissions/:id/unlock` that reverses Final Submit (status → `in_progress`, `submittedAt` cleared, `readerLock` cleared) and writes a `submission.unlock` audit entry with `priorStatus` + `priorReaderLock` + reason. The reader-side `DELETE /api/submissions/:id/lock` only clears a reader lock; this is the system-lock override. 6 admin-unlock integration tests passing (incl. lockout-middleware-lifts after unlock).

S2A.2 / **CR-008 Phase 1**: new `GET /api/submissions/:id/preflight` returning `{ submitDisabled, errors[], warnings[], counts }` mirroring the submit gate exactly — error codes `NARRATIVE_MISSING`, `VALIDATION_FAILED`, `NOT_EVALUATED`; warning `MANY_EXCLUDED` (>25% N/A). `FinalSubmitModal` consumes the response via new `preflight`/`preflightLoading`/`onGoToSpec` props, disables Submit when errors exist, and renders a "Go to" button per error that jumps to the spec in the editor + closes the modal. Pinned by 7 preflight-endpoint tests + 8 FinalSubmitModal unit tests. Override-with-reason (audited PC override of the gate) deferred to Sprint 2B.

**Vault**: CR-005 + CR-006 promoted to `shipped`. CR-050 promoted to `shipped`. CR-008 stays `in-progress` (Phase 1 only). Sprint plan section updated. Total Sprint 2A test coverage: 34 new server integration tests + 13 new client unit tests, all green; full server suite 282 → ~316 passing.

## [2026-05-30] update | CR-049 Phase 5 — Sprint 2.5 finish: hints_fn against live Qdrant

ai-service: `app/main.py` `_build_section_hints_fn` builds a closure that calls `retrieve_for_section` (from `app/corrections/store`) scoped to `(institutionId, programLevel)` and filters hits to the `(std, spec)` under evaluation; formats via `format_examples_for_prompt`. Wired into `/ai/section/evaluate` via `evaluate_section(..., hints_fn=hf)`. Env-gated by `Settings.enable_section_eval_hints` (default True; `ENABLE_SECTION_EVAL_HINTS=false` opts out). Fail-soft on any Qdrant / embedding failure — evaluation still completes without hints. The narrative-HTML is stripped to text for the semantic anchor (embeddings are text-domain). `SectionEvaluateRequest` gains `programLevel` (default bachelors).

server: `cshseAiClient.SectionEvaluateRequest` adds required `programLevel` ('associate' | 'bachelors' | 'masters'); `ValidationService.validateSection` looks up the submission's `programLevel` and threads it to `evaluateSection(...)` so the corrections RAG is properly scoped at the institutional + programLevel boundary.

Tests: `test_section_eval_endpoint.py` +4 — (1) the prompt carries the formatted hint block + the `section_eval_override` correctionType when retrieval returns a hit; the retrieve call is scoped per-institution + per-programLevel; the narrative anchor is HTML-stripped; (2) `ENABLE_SECTION_EVAL_HINTS=false` short-circuits retrieval; (3) Qdrant throwing inside the hint lookup does not break the evaluation; (4) empty `institutionId` returns `None` hints_fn (no leak). ai-service full suite **404 passed / 5 skipped**; server CR-049 files **33/33** green after the programLevel thread.
...
## [2026-05-30] update | Sprint 5.1 — CR-009 Compilation tab shipped

Lead-reader compilation surface — first major piece of the lead-reader workflow. Side-by-side 0-3 scores across all readers per (std, spec), with the highlight rules from the webinar slide: red on any 0, yellow on disagreement, green on all-agree, muted for excluded (CR-050). Lead reader stamps a Final score inline; both transitions audit-logged.

**Architecture decision — source of truth:** the new compilation reads `Score` (0-3, written by Sprint 3 readers) directly, NOT the legacy `LeadReaderCompilation` aggregator which reads `Review.assessments` compliance triplet. The legacy aggregator is untouched (still serves the older flow) but unused for this view; CR-009 acceptance is explicit in 0-3 terms ("any reader scored 0"), so reading `Score` is the only way to get it right.

Server: new `models/LeadFinalScore.ts` (`{ submissionId, standardCode, specCode, score (0..3), setById/setByName/setByRole, note? }`, indexed unique on (sub, std, spec)) — a dedicated collection sidesteps the Mongoose-Map dotted-key trap and keeps audits trivial. `controllers/compilationController.ts` — `getCompilation` (lead/admin only; returns rows + readers with hasZero/hasDisagreement/allAgree precomputed, plus excluded passthrough from `standardsStatus`), `setFinalScore` (lead/admin; upserts; audit `compilation.final_set` with priorScore), `clearFinalScore` (audit `compilation.final_cleared` with priorScore). New `routes/compilation.ts` mounted at `/api`. `AuditAction` widened.

Client: pure `CompilationTabView` (`features/leadReader/CompilationTab/`) — table with one column per reader plus a Final column. Final cell shows the value as a chip (or `— not set`) with Set/Change/Clear; clicking Set/Change inlines `Score4LevelSelector` (reused from Sprint 3) + Save / Cancel. Container wires the `compilation` GET query + `setFinalScore` / `clearFinalScore` mutations, invalidating the query on settle. Pure `LeadReaderDashboardView` (`features/leadReader/LeadReaderDashboard.tsx`) — lists submissions over `GET /api/submissions` (server's CR-007 gate handles role filtering). Page wrappers + routes `/lead-reader` and `/lead-reader/:submissionId` in `App.tsx`. Layout nav adds "Compilations" for lead_reader / admin / superuser.

Tests: server `tests/integration/compilation-endpoint.test.ts` (13 — hasZero/hasDisagreement/allAgree flags, excluded passthrough, PC/reader 403, admin 200, 404 on unknown sub, upsert + audit fires + priorScore on subsequent update, invalid score 400, clear + audit). Client `features/leadReader/CompilationTab/CompilationTab.test.tsx` (12 — loading/error/no-rows/columns, row data attributes for highlight bucket, excluded hides action buttons, beginEdit fires with key + current, editor renders the 4-level selector + Save fires, Save disabled with no pending, Clear fires, legend). Client `features/leadReader/LeadReaderDashboard.test.tsx` (4 — loading/empty/row links/status labels). Full client 259/259; full server 331/331.

Vault: CR-009 promoted to **shipped** (DOCX export deferred to Sprint 5.2 alongside CR-011). Sprint plan S5.1 marked done.

## [2026-05-30] update | Sprint 5.2 — CR-011 Suggestions DOCX shipped

Lead-reader / admin can now export a consolidated reader-suggestions DOCX from the Compilation tab, with a two-mode toggle: **Internal** (board / VP — reader names + raw content + override notes + AI suggestions visible) and **PC-facing** (anonymized — unrelayed comments dropped entirely, relayed comments shown as `pcLabel` + `relayedText`, reader override notes suppressed, AI suggestions retained).

Server: `services/suggestionsDocx.ts` builds the DOCX with `docx` package — cover page + per-standard sections + per-spec subsections (Reader comments / Override notes / AI suggestions). Pulls from `Comment` (relay-aware filtering), `ValidationResult` (latest per spec; reader override note + AI `suggestions[]` + unmet-criteria `criteriaCoverage[].note`). `compilationController.exportSuggestionsDoc` mounts at `GET /api/submissions/:id/compilation/suggestions-doc?mode=internal|pc_facing` (lead/admin only; reader+PC 403; unknown mode falls back to internal). Sets `Content-Type: wordprocessingml`, `Content-Disposition: attachment; filename="suggestions_<inst>_<mode>_<date>.docx"`, and `X-Suggestions-Mode` for the client to observe.

Client: `CompilationTabView` gains an export toolbar (mode radios + Generate button) immediately under the page header. Container uses `axios` with `responseType: 'blob'`, wraps the response as a `Blob` if needed, and triggers download via `URL.createObjectURL` + a transient `<a>`.

Tests: server `tests/integration/suggestions-doc.test.ts` (7) seeds an unrelayed comment ("Jane Reader"), a relayed comment with `pcLabel: 'Reader A'` + sanitized text + a CONFIDENTIAL raw body, and a reader-overridden ValidationResult with a note + AI suggestions. Asserts the redaction round-trip by unzipping the response buffer with `jszip` and grepping `word/document.xml` — internal mode contains the names + raw content + override note; pc_facing mode contains NONE of those, only `Reader A` + sanitized text + AI suggestion. Plus role gates (PC + reader 403, admin 200), 404 on unknown submission, unknown-mode fallback. Client `CompilationTab.test.tsx` adds 3 tests (toolbar render / mode toggle / Generate fires + disabled-while-exporting). Existing CompilationTab tests updated for the wider props shape; all 15 green.
...
## [2026-05-30] update | Bug fixes — Introduction card wiring + Upload Files always shows Upload + CR-051 proposed

Three UX issues raised inline during the Sprint 6 review window. Two fixed, one promoted to a new CR.

**Bug 1 — Introduction card click left the right pane empty + no edit affordance.**
Root cause: `findItem` in `ItemPreview.tsx` searched buckets / tags / cvs / evidenceDocs but never the `introductions` Record. So a click set `selectedSectionId` in the store, `findItem` returned null, and the right pane stayed on its "Select an item from the middle pane" empty state. Pencil affordance also wasn't whitelisted for `kind === 'introduction'`, so even if the right pane had rendered there was no way to edit. Fix: added `introductions?` prop to `ItemPreview`, a search branch in `findItem` that resolves intro items as kind='text' (they share the BucketItem shape with narratives), threaded `introductions` from `ReviewStep` into the `<ItemPreview>` mount, added `editIntroductionItem` + `revertIntroductionItem` to `aiImportStore` (walks every intro bucket since the caller has only sectionId; mirrors `editBucketItem`'s originalSnippet + editedAt bookkeeping so the existing "edited" badge + Revert button light up the same way), routed `handleEditSave` / `handleEditRevert` in `ReviewStep` through a new `isIntroductionSection` helper, and added `'introduction'` to the pencil-button kind whitelist in `ItemCardList.tsx:1220`.

**Bug 2 — Clicking "Upload Files" toolbar with a parse in flight kept showing the in-flight Parse step with stale "Parsing complete" data.**
Root cause: `SelfStudyEditor.tsx:299` only reset to upload step when the prior run had SETTLED (parsed / applied / finished / failed / canceled). For an in-flight `parsing` run, the wizard re-opened pinned to Parse. The original intent was to preserve live progress; UX feedback says "Upload Files" should always mean "show me Upload". Fix: drop the `settledRun` guard; `startOver()` runs unconditionally. The Parse step is still reachable via the sidebar step picker if the coordinator wants to switch back to watching the run.

**Question 3 — "Where's the View button on CV / Syllabus / Paper cards?"**
...
## [2026-05-30] update | Sprint 8 — CR-019 Joint Venture grouping shipped (revived from rejected)

User direction 2026-05-30 ("Sprint 8/CR-019 should not be dormant. This is an expected requirement. Work it now.") overrides the 2026-05-20 rejection. Shipped end-to-end the same day following the [[sprint-plan-2026-05-11#sprint-7]] full spec.

**S8.1 — Model + REST API.** New `JointVenture` model (name unique; description optional; institutionIds with ≥2 enforced at the controller; archived/archivedAt/archivedReason; createdBy/createdByName). `Institution.jointVentureId` reverse pointer (indexed) maintained by the controller — set on create + add member; cleared on remove + archive. `jointVentureController` exports list / get / create / update / addMember / removeMember / archive / aggregateStats. RBAC: admin/superuser full CRUD; non-admin (PC/reader/lead) GETs only the JVs whose member institution they belong to (non-member 404 — don't leak existence per the original CR spec). Invariants pinned: name duplicate → 409, institution already in another active JV → 409 (on both create + add), remove that drops <2 → auto-archive with audit `jv.archived` (payload `reason: 'auto'`) + reverse pointers cleared on both removed institution AND any surviving members. `AuditAction` union widened with `jv.created` / `jv.updated` / `jv.member_added` / `jv.member_removed` / `jv.archived`.

**S8.2 — Admin client.** `features/admin/JointVentureManagement/JointVentureManagement.tsx` — pure view + container; list active / archived JVs (per-row name + member count + member names + Archive button on active rows); inline create form (name + description + institution multi-select filtered to institutions with no `jointVentureId`); Create disabled until name set + ≥2 institutions picked. Mounted at `/admin/joint-ventures` via `AdminPage`.

**S8.3 — Aggregate stats + badge.** `GET /api/joint-ventures/:id/aggregate-stats` returns `{ memberCount, stats: { totalSubmissions, activeSubmissions, decidedSubmissions, upcomingSiteVisits } }` rolled up across members. New `JointVentureBadge` component (small `Network`-icon chip "Joint Venture: {name}"; reads the same per-viewer list endpoint then filters client-side — so badge only renders when the viewer actually has visibility into the JV per server RBAC). Mounted on the reader dashboard rows next to the institution name. `submissionController.listSubmissions` projection widened to include `institutionId` so the badge has its key.

Tests: server `tests/integration/joint-ventures.test.ts` (12) — create with min-2/duplicate-name 409/cross-JV conflict 409/admin-only CUD with PC + lead 403; add member sets reverse pointer; add of already-in-another-JV institution 409; remove that drops <2 auto-archives + clears both pointers + fires both `member_removed` AND `archived` audits; RBAC (member 200, non-member 404, no leak); list filter (non-admin per-member; admin sees all); aggregate-stats across 2 members (1 active + 1 decided); manual archive with reason + audit reason + reverse pointers cleared. Client `JointVentureManagement.test.tsx` (11) + `JointVentureBadge.test.tsx` (4). Full client suite **374/374** (was 359 + 15 new).

Deferred: dashboard JV section grouping (admin / lead-reader), reporting filter dropdown + `?jointVentureId` on existing report endpoints, PC dashboard badge mount. Data + endpoints + component are live; each is a discrete UI pass when product confirms the surface.

Vault: CR-019 promoted from `rejected` → **shipped** (revision_history entry documents the revival rationale); sprint-plan-2026-05-29 Sprint 8 section rewritten from "dormant" → done with S8.1/S8.2/S8.3 entries; index row updated.

## [2026-05-31] update | Sprint 9 — Notification pass (CR-010 / CR-053 / CR-052 / CR-021)

Cohesive multi-CR sprint chosen from the [[sprint-plan-2026-05-29]] §4 follow-ons. All four CRs were already `shipped`; this extends each — `shipped_notes` added to all four CR pages + catalog rows updated. Built on `developer`, not pushed (wizard-run constraint).

**S9.1 — Notification core + CR-010 DM notifications (`6496aba`, `ca35031`).** New foundation the rest of the pass builds on. `server/src/models/Notification.ts` — in-app recipient inbox (`recipientId` ref User, `type`, `title`/`body`/`link`, optional `submissionId`, `read`/`readAt`, `dedupeKey`); two indexes incl. `{recipientId, dedupeKey}` unique partial for idempotent producers. `server/src/services/notificationService.ts` — `notify()` (dedupe pre-check → create row → catch E11000 on race → fail-soft `_sendEmail` twin via existing emailService) + `notifyMany()` (dedupe recipients, fan out). `server/src/controllers/notificationController.ts` + `routes/notifications.ts` — per-recipient list (`?unreadOnly`, limit clamp, offset) / unread-count / mark-read (scoped to owner → 404 if foreign) / mark-all-read; mounted in `index.ts`. `directMessageController` `createThread` + `postMessage` now call `notifyMany` for every OTHER participant (sender excluded), type `dm.new_message`, link `/reader/{submissionId}`. Client `NotificationBell` (pure view + container) in the Layout user-menu — unread badge (99+ cap), 30s unread-count poll, list loads on open, click marks read + navigates. Tests: 10 server integration + 8 client view-unit; DM regression 9/9; full client 382/382.

**S9.2 — CR-053 board cycle reminders (`f9bda21`).** Admin-only `POST /api/board/run-cycle-reminders?withinDays=N` (default 180, clamp [1,1825]) in `boardDecisionController.runCycleReminders`; scans expiring accepts (`decision.expiresAt` in window) + tabled-due (`decision.reconsiderAt` in window), fans out to every admin/superuser via `notify`. Idempotent — `dedupeKey` = `{prefix}:{subId}:{dateKey}` (date sliced to YYYY-MM-DD), so re-runs no-op and a re-decided expiresAt yields a fresh reminder. No in-process cron — external scheduler / manual trigger. Returns `{withinDays, scanned:{expiring,tabled}, adminRecipients, notificationsCreated}`. 3 server integration tests; board-decisions regression 8/8.

**S9.3 — CR-052 per-feature first-time hints (`de5da94`).** HintProvider is in-memory (session-only dedupe), so added `client/src/features/tour/useOnceHint.ts` — a localStorage "seen" ledger (`cshse:hint-seen:{id}`, fail-soft) that fires a hint once-ever per browser. Wired two nudges: first Final-score edit on CompilationTab (fired from an effect on `editingKey` so the Save-button anchor is committed before HintLayer measures it via getElementById — HintLayer falls back to a centered balloon if the target is missing and only re-measures on resize/scroll, hence the effect) and first checklist verify (not un-verify) on the site-visit Checklist. Real `id` attributes added to both anchor buttons. Two `hint.*` strings (five-year-old voice). 4 client unit tests; CompilationTab 15/15 + Checklist 10/10 regression.
...
## [2026-05-31] update | CR-052 help coverage — first-time hints wired into new surfaces (CR-010 Messages "Conversations" header, CR-022 locked assignment-change box); id anchors + hint strings + anchor-presence tests; client 27/27 on touched specs. commit cde411f
## [2026-05-31] update | CR-008 closed shipped — override-with-reason verified live (FinalSubmitModal + submissionController.ts:1662+ override bypass + final_submit_override audit branch); frontmatter + catalog row + body closure note reconciled
## [2026-05-31] update | CR-054 proposed — n8n validation migration + callback security; splits out the S14 deferred-to-CR n8n line; Phase-1 security fix (auth/404 callbacks, rotate hardcoded key, bind help-chat session) ships independently of Phase-2 editor-validation re-point + Phase-3 ~600-LOC removal

## [2026-06-01] update | CR-055 + CR-056 shipped — Assignment producer + lead-reader send-to-board (commit 0b2c5ee, deploy b2968d10)
CR-055: `assignReaders` now reconciles ACTIVE `Assignment` docs (one per reader/lead, with `assignmentType` + `leadReaderId/Name`, dropped readers marked `removed`), stamps `submission.leadReader`, so assigned reader-shaped roles pass the Assignment access gate. CR-056: new `POST /api/submissions/:id/compilation/finalize` (lead/admin) → `review_complete` (audits `compilation.finalized`, notifies admins `board.ready`); SendToBoardPanel wired into CompilationTab. Also fixed two pre-existing develop build breaks (`excluded`/`excludedReason` on SelfStudyEditor type; `continuous` in CshseTooltip test mock) that had failed every develop deploy since 05-29 — b2968d10 is the first green develop deploy since then. Tests: cr055 (3) + cr056 (8) server + CompilationTab (27) client green; full server suite 526/528 (the 2 failures are pre-existing flaky notification-timing/logout-404 tests, green in isolation).

## [2026-06-01] setup | Stevenson (2026-001) primed for beta walkthrough — approve+apply, 4 users, submit, assign → readers_assigned
Live data setup on cshse-develop: approved all 68 AI-review items (15 CVs + 53 evidenceDocs) and applied (review state drained, 6 supporting-evidence records). Provisioned 4 users (pw `Walkthrough#2026`): reader1@cshse.test, reader2@cshse.test, lead-assigned@cshse.test (reader panel), lead-unassigned@cshse.test (separation control). Final-submitted with CR-008 override; assigned reader1+reader2+lead-assigned → status `readers_assigned`, `leadReader` stamped, 3 ACTIVE Assignments + 3 reviews. Separation verified live: unassigned lead GET submission → 403, assigned reader → 200. Stevenson left at `readers_assigned` for the walkthrough (NOT advanced to board).

## [2026-06-01] audit | CR-057 full reader→lead→board pipeline verified live + committed regression guard
Ran the complete chain on a throwaway submission against cshse-develop: assign → reader scores → lead final scores → finalize (review_complete) → board queue lists it → admin Accept → compliant → reader PDF (application/pdf, 3305 B) + suggestions DOCX (94 KB) — 16/16 checks green, throwaway self-cleaned, Stevenson untouched. Committed durable counterpart `server/tests/integration/cr057-reader-lead-board-chain.test.ts` (2 tests) green. Beta-walkthrough chain proven end-to-end. NOT pushing to main until after the Eric/Julia/Lauri walkthrough.

## [2026-06-01] update | CR-054 shipped — n8n validation retired + callback security closed (all 3 phases)
Implemented all three phases on branch `developer`. **Phase 1 (security):** added env-gated `server/src/middleware/verifyN8nCallback.ts` (shared-secret guard via `X-Callback-Secret`/`X-API-Key`, constant-time compare; pass-through until `N8N_CALLBACK_SECRET` set so live spec-loader/document-matcher workflows don't break); bound help-chat Redis memory to `sha256(userId:clientSessionId)` in `helpChatController.sendChatMessage` (closes audit C5); replaced the hardcoded spec-loader workflow key with `{{ $env.CSHSE_CALLBACK_KEY }}` in `n8n-workflows/cshse-specification-loader-pdf.json`. **Phase 2 (migration):** editor "Validate" now calls in-process cshse-ai — `client/.../useValidationStatus.ts` posts to `/api/webhooks/validate` → new `webhookController.runSectionValidation` → `validationService.validateSection`. **Phase 3 (delete):** removed the `/api/webhooks/n8n/{validate,callback}` routes + `triggerValidation`/`processCallback`/`callWebhook`/`validateStandard`/`revalidateFailedSections` (~570 LOC net). Headline C2 exploit (unauth validation-callback result tampering) closed by **deletion** of the callback. Inverted `webhook-callback-security.test.ts` from documenting-broken-state to asserting secured behavior (7 tests green). **Deferred ops:** set `N8N_CALLBACK_SECRET` (Railway) + `CSHSE_CALLBACK_KEY` (n8n) to enforce the remaining spec-loader/document-matcher callback guard.

## [2026-06-01] audit | Comprehensive E2E regression — server 533 + client 417 automated tests
Full automated regression after CR-054. **Server vitest:** 533 tests / 59 files — 531 pass on the parallel run; 1 was my own CR-054 assertion (n8n callback expected 404, gets 401 via the `authenticate` fall-through — both are rejections; assertion relaxed, now green) and 1 was a `comment-relay.test.ts` escalate **test-isolation flake** (passes 16/16 in isolation; a parallel worker's cleanup removed the seeded comment — pre-existing test-infra behavior, NOT a product bug and NOT touched by CR-054). **Client vitest:** 49 files / 417 tests — all pass. Net product regression count after the flake/assertion accounting: 0 failures. Playwright E2E suite (~36 specs) requires a local full-stack (Mongo+S3+n8n on :3000) and is functionally covered by the server/client suites + the live reader→lead→board API chain.

## [2026-06-01] update | CR-058 shipped — impersonation audit logging (built while verifying "impersonation log functionality")
User asked me to verify impersonation-log functionality on the live `develop` env. Found there was **none**: server impersonation was role-override-only via the `X-Impersonated-Role` header, the impersonated *user* identity never reached the server, and `AuditLogEntry` had no impersonation action/field. User chose "build it." Implemented on branch `developer` (CR-058): (A) new `server/src/middleware/requestContext.ts` `AsyncLocalStorage` impersonation context, populated in `authenticate` from `X-Impersonated-Role`/`-User-Id`/`-User-Name` and bound around `next()` so it covers the whole request; (B) `recordAuditEvent` auto-reads that context and writes a new optional `impersonation` sub-document on `AuditLogEntry` — **zero churn to the ~9 existing audit callers**; the top-level actor stays the *effective* (impersonated) identity, the truth lives in the block; (C) new `auth.impersonation_start|stop` actions + superuser-only `POST /api/auth/impersonation/{start,stop}` (public router, inline jwt.verify), called fire-and-forget by client `start/stopImpersonation`; (D) admin AuditTrail renders an amber "impersonated by <SU> as <role/user>" badge + CSV gains `impersonatedBy`/`impersonatedAs`. Client `api.ts` now forwards the impersonated user id + URI-encoded name. Tests: `server/tests/integration/cr058-impersonation-audit.test.ts` (9), `audit-trail.test.ts` CSV-header assertion updated, `AuditTrail.test.tsx` (+2), `authStore.test.ts` mock widened for the new POST. Note: CR id is **058** — 057 was already taken by the reader-lead-board pipeline test doc.

## [2026-06-05] ingest | CR-059 proposed — import individual file into the Self-Study editor (standalone, replaces original editor)
User direction (2026-06-05): while in the Self-Study editor OUTSIDE the AI Import Wizard, the PC needs to import one file and copy/paste its content into the exact spec/sub-spec — as the standard narrative OR as supporting evidence (CV/syllabus/project) — the way the pre-wizard original importer did. Authored `cr-059-import-individual-file-into-self-study-editor.md` (status **proposed**, P1, Sprint 11 target) via a 4-agent research workflow that mapped the real reuse surface before writing: drag/drop+browse from `FileUpload.tsx`, inline parsed preview from `FilePreviewModal.tsx` over `documentParser.ts` (`GET /…/evidence/:id/preview`), imperative `insertHtmlAtCursor` on `NarrativeEditor.tsx` → existing autosave `PATCH /…/narrative`, and the existing `SupportingEvidence` create/list flow for the "keep as evidence" action. New client `ImportFilePanel.tsx`; NO server change, NO AI. Includes removing the legacy "Import Document" toolbar button + the CR-045 "Hide legacy importer" settings toggle (this panel replaces them). Added the P1 index.md row. Line-number anchors flagged as approximate (verify-before-implement). Test plan names new e2e specs 47–50.

## [2026-06-05] update | CR-059 clarified — 3 open questions resolved by user
User answered all three CR-059 open questions: (1) panel reachable with NO sub-spec selected — yes (standard level + Introduction surface; narrative paste target adapts, evidence-summary target attaches at standard level with specCode blank); (2) section selection = native browser text selection (v1); (3) NOT an explicit "keep as evidence" step — every uploaded file is auto-treated as an imported file (SupportingEvidence created on upload, shows in File Library immediately), and the PC pastes selected text as a SUMMARY into either the narrative OR the file's supporting-evidence description (independent, non-exclusive). Rewrote the CR Decision/Acceptance, folded the open questions into a "Resolved decisions" section, added IntroductionEditor to files-affected, and renumbered the test-plan e2e specs (47 dragdrop+auto-import, 48 paste-into-narrative, 49 paste-evidence-summary, 50 no-spec-selected, 51 legacy-removed). Updated the index.md row. Still status **proposed** — awaiting go-ahead to implement.

## [2026-06-05] update | CR-059 shipped — Import-file drawer in the Self-Study editor
Implemented + live-E2E-verified CR-059. New `ImportFilePanel.tsx` right-side drawer (drag/drop or browse one file → auto-imports as SupportingEvidence (File Library) → inline parsed preview → native text-select → "Paste into narrative" OR "Paste as supporting-evidence summary"). NarrativeEditor/IntroductionEditor expose an imperative `insertHtml` via an `onEditorReady` callback (insert + immediate save). Removed the legacy "Import Document" toolbar button + the CR-045 "Hide legacy importer" settings toggle; the legacy modal lifecycle is left inert (deletion deferred). Playwright specs 47–51 (+ sample docx fixture + importFile helpers) all green on cshse-develop. Regression suite 39/40/41/44/45/46 green. NOTE: regression surfaced a PRE-EXISTING Review change-kind autosave/load race (spec 43) — CR-059's bundle timing tipped a latent race to failing; point fixes either regressed other specs (load clobber-guard) or were insufficient (immediate kind-save, kept as defensive). Flagged for a dedicated CR to redesign the Review autosave. Commits: feat 000d1a3, narrative-persist e5e79e5, two reverted experiments + surgical immediate-save 015ae57.

## [2026-06-05] update | Review change-kind persistence race FIXED + reader/lead-reader persistence verified
Fixed the Review-rail kind-flip (narrative→evidence) persistence race that CR-059's bundle timing had exposed (spec 43). Two compounding causes, both fixed: (1) late-load clobber — `loadPersistedReviewState` now keeps local `buckets` only when their object identity changed during THIS GET (minimal, per-fetch, byte-identical to original for every other field; the earlier dirty-based guard regressed 39/40/41/44/45/46 because the seed persists dirty:true); (2) mount-vs-edit write race — `saveReviewStateToServer` now serializes all save-state POSTs through one promise queue so the stale mount autosave can't land after the edit autosave. An immediate-save in handleChangeKind was tried + removed (it stuck the Saving… chip). Diagnostic confirmed server holds the item in evidenceText after flip AND reload. Spec 43 green (3/3 sequential, 15/16 full parallel suite; lone miss = 15s UI-render timeout under artificial 4× load, not data loss). Reader + lead-reader persistence tested live: new specs 52 (reader Score persists + re-score upsert) and 53 (lead LeadFinalScore persists + clears) pass; plus the existing 75-test reader/lead-reader integration suite green. Full PC persistence suite (37–51) + 52/53 green. Commits: buckets-guard 8870075, immediate-save-removal 7276b03, serialization 2b5576b, reader/lead specs 9d6ad76.

## [2026-06-07] update | Review state moved off localStorage → server-authoritative; full E2E green
Per user directive ("nothing in localStorage except the document/comparison items"): audited every localStorage user. Only offender was aiImportStore's `ai-import-storage` persist, which carried the whole review state (buckets/narratives/tags/cvs/evidenceDocs/introductions/matrices/matrixRowEdits/approvedIds/discardedIds/coverageReport/dirty). Removed all of it from `partialize` (kept only resumable identity). Made persistence STORE-LEVEL (useAIImportStore.subscribe → debounced serialized saveReviewStateToServer) so every edit hits the DB regardless of surface (wizard ReviewStep OR ReviewSurface); chip reads shared `reviewSaveState`; removed ReviewSurface's local effect. SelfStudyEditor now hydrates the store from Submission.aiReviewState on mount so the toolbar Review button + counts work without the localStorage cache. authStore (JWT) kept — credential, not data. Total live E2E 17/17 (PC 14 + Reader 52 + Lead 53 + new leanness check 54) green; server integration 37/37 green. Commits 48633b6 (refactor), 3901b74 (editor hydration), 3498a95 (leanness spec). Caveat: wizard mid-import resume needs the local stack to exercise (not run live); user's existing submission migrates its localStorage narratives to the server on next editor open (recovery).

## [2026-06-07] update | Dashboard DRAFTS now mirror the Review rail (totals) + full E2E green
User reported the dashboard still looked wrong + suspected data wasn't pushed. Verified the live submission (6986239a…) server aiReviewState now holds 207 narr + 66 evText + 9 evFiles = 282 + 15 cvs + 41 evidenceDocs (30 syll/11 paper) + 1 intro + 68 approvedIds — i.e. EVERYTHING matches the Review screen; the recovery fully migrated the localStorage narratives to the DB. The remaining mismatch was display semantics: DRAFTS tiles showed un-triaged (CVs 0 because all 15 approved). Per user direction, changed getWorkflowSummary DRAFTS counts to TOTALS (mirror the Review rail) + added drafts.reviewed for an "X reviewed" progress hint; WorkflowSummary subtitle "Waiting in Review"→"In review (totals)". Verified live for the user's submission: dashboard now CVs 15 / Syllabi 30 / Projects 11 / Intro 1 / Spec items 282 (== rail). Full live E2E 18/18 (37–55, incl. new 55_dashboard_totals + 54_localstorage_lean). Commit e8a1fc7.
...
## 2026-06-09 (cont.) — Reader Report now fills the OFFICIAL CSHSE template per degree level

**Correction:** the first reader-report generator produced an ad-hoc layout. The reader report is the official CSHSE **compliance checklist** (one template per degree level), so the template must be filled in — not reproduced.

- **Templates:** the July-2025 official files (`CSHSE/docs/…Associate/Baccalaureate/Master's…docx`) are per-standard grids with Compliant / Non-Compliant columns + a Reader's Comments column + recommendation/signature block.
- **Prep:** `scripts/prepare_reader_report_templates.py` (python-docx) inserts `{{c_N}}/{{n_N}}/{{cm_N}}` placeholders into each numbered standard's Compliant/Non-Compliant/Comments cells + `{{inst_name}}/{{prog_name}}` header fields → placeholder-tagged copies in `server/src/assets/reader-report-templates/{baccalaureate,associate}.docx`. Fillable rows detected in document order; an ordered key list assigns each to a standard (intro/conditional rows left blank for the reader).
...
## 2026-06-11 — Header stats fix + table formatting audit + reader-view tables

- **Header stats** (SelfStudyEditor): "X/86 Standards" counted all standardsStatus keys (65 specs + 21 standard-level) — ProgressIndicator now counts STANDARDS only (addressed = has content / complete) → 19/21 on Stevenson. "X/83 Validated" counted only validationStatus==='pass' (4) — now counts specs with ANY verdict (reviewed) → 65/83; submit gate (all-pass) unchanged; disabled-Submit tooltip clarifies passed vs reviewed.
...
## [2026-07-24] ingest | AI parser architecture + self-improving parser agent
Created: [[ai-parser-architecture]] (concept), [[change-requests/cr-073-self-improving-parser-agent]] (change-request, proposed).
Updated: index.md (parser architecture under Architecture & systems), change-requests/index.md (CR-073 row).
Context: Documented the complete cshse-ai parser architecture at Eric's request — the three input formats (template tabular/paragraph, self_study, mcc_narrative), format detection (incl. the table-cell folding that fixed tabular misclassification), the three pipelines, how the level-specific CSHSE standards drive routing + the level-aware rubric, and the **required-output contract**: buckets/introductions/tags + per-item shape + routing invariants + the **`data-section-id` anchor required on EVERY item** (the tag whose absence caused the recurring Compare "section not located" bug; now enforced by `_ensure_all_items_anchored` + the `reanchor-source` endpoint + e2e/72). Included the failure-mode table (8 rules discovered on AACC). CR-073 specs a new agent that runs the parser, validates against that contract, diagnoses failures, and refines the parser via two channels — deterministic rules+fixtures (human-gated code) and per-institution Qdrant few-shots (auto) — so any self-study yields a correct Review panel that Compares against the original, and the parser compounds smarter with each document. Automates the manual AACC diagnosis loop (memory log 2026-07-17→23).

## [2026-07-24] ingest | AACC parser fixes + Compare anchor-tag repair (worked example)
Created: [[ai-parser-aacc-fixes-2026-07-24]] (review).
Updated: index.md (reviews section).
Context: Documented the table-by-table AACC diagnosis at Eric's request — the eight parser defects + rules/fixtures (commits c2060e1/7dd86b5/941c4bf/96215a7/5b690aa) and, in depth, the Compare "tags" repair (commits b8cc2c3/93f16d6): the marker is `data-section-id="<item.sectionId>"` on a `<section>` in the stored source HTML; only the MCC pipeline wrote them, so template imports showed "section not located — showing top". Rule: EVERY item carries a marker (user: "all documents must have them no matter what section they fall in"); `_ensure_all_items_anchored` for new imports + `POST /api/imports/:id/reanchor-source` to repair existing without re-parsing. Reanchored all prod imports, 0 missing (Kennesaw 161, MCC 169, AACC 339). This is the reference loop [[change-requests/cr-073-self-improving-parser-agent]] automates. PENDING: fold in the agent-build METHOD from Sydney Runkle's X article (auth-gated — could not fetch; asked Eric to confirm which method).

## [2026-07-24] ingest | Agent build method — Graph/Loop engineering (adopted)
Created: [[agent-graph-loop-engineering]] (concept).
Updated: [[change-requests/cr-073-self-improving-parser-agent]] (Build-method section), [[ai-parser-architecture]] (method reference), index.md.
Context: Eric shared "Graph Engineering vs Loop Engineering" (aibuilderclub.com) as the method for building the parser training agent. Core: a graph imposes your knowledge as a flowchart (boxes = code/LLM/tool, arrows = fixed or conditional) so wrong paths are impossible and the LLM reasons only where it adds value; a loop is the smallest possible graph (one node self-cycling); agent graphs are cyclic not DAGs (retry/fix/human-approval); the VERIFIER (not the model) is the bottleneck — master the node before wiring the graph; predictable workflow → graph, open-ended → loop. Applied to CR-073: the parse-QA workflow is predictable → build as a graph (nodes: parse, six parallel §7 contract checks, diagnose, structural-refiner + semantic-refiner, human-approval gate; conditional routing by finding type; cyclic refine→parse; dynamic fan-out), with the [[ai-parser-architecture]] §7 required-output contract as the agent's verifier/exit test. The X article by Sydney Runkle (auth-gated) links to this same method.

## [2026-07-24] update | CR-073 — variety + hybrid documents made a first-class requirement
Updated: [[change-requests/cr-073-self-improving-parser-agent]] (Scope table of the five document shapes incl. combinations/hybrids; acceptance adds a hybrid fixture; Open question 5 on hybrid handling — per-region sub-parsers reconciled into one review state + one anchored source HTML).
Context: Eric — "read the document, do a complete analysis like AACC, create/update parsing rules, and generate output that works with Compare. We get a variety: some match specs exactly, some MCC, some AACC, some Kennesaw, but could also be a combination." Documented the requirement that the agent handles all shapes AND hybrids per-region.

## [2026-07-25] update | Prime requirement — realtime learnable parser rules (data, not code)
Updated: [[ai-parser-architecture]] (§7f Success metric = right place + right file type + clean Compare; §9 the learnable layer — engine=code / rules=data in a Mongo `parserRules` store + the realtime learning loop), [[change-requests/cr-073-self-improving-parser-agent]] (Success definition; "What the realtime learner requires" = input signature + ground truth + gap + rule&validation, with the full `parserRules` schema; three learning channels — structural rules now REALTIME data; new files ParserRule model + rule_engine.py).
Context: Eric — the prime AI-parser requirement is that it LEARNS from each document and improves IN REALTIME (rules stored in a Mongo table), not via code deploy. Answered "what information is required to improve the parser in realtime": the four things the agent captures per mis-handled region (structural signature, ground truth from doc×catalog, contract gap, the synthesized rule + contract re-check). Success formalized as the three per-card criteria: correct standard/sub-spec placement, correct file type (appendix/syllabus/CV/matrix), and Compare with no missing/errored compares (every data-section-id marker resolves to "located"). Engine stays code; shapes become data rows the agent writes and re-parses with, so the current doc self-corrects and future docs inherit — code only for genuinely new primitives.

## [2026-07-25] update | CR-073 → "Parser Train" SU function (human-in-the-loop via the Review screen)
Updated: [[change-requests/cr-073-self-improving-parser-agent]] (renamed to Parser Train; new §"Parser Train — the superuser function"; Open questions 1 & 3 DECIDED; acceptance adds SU-only + isolation + verify-in-Review + approve=activate-rule; files add ParserTrain server/client + parserRules; human-approval node = SU spec approval), change-requests/index.md.
Context: Eric — make it a superuser function called **Parser Train**: read + analyse the document, set the parser rules, and use the existing Review screen (SU-only) as the verification surface to inspect issues and run Compare. Approving the spec/sub-spec approves the rule for parsing (the human-in-the-loop). SU can add notes + attach screenshots on wrong cards. Already-parsed documents (MCC, AACC, Kennesaw State) must NOT be affected — sandbox training runs; activated rules apply to future imports only.

## [2026-07-25] update | Parser baseline capture + golden regression tests (the safety net)
Updated: [[change-requests/cr-073-self-improving-parser-agent]] (new §"Baseline capture + golden regression tests"; acceptance + files add baseline rule capture, golden fixtures, guardrail), [[ai-parser-architecture]] (§9 — proven docs = the baseline/floor).
Context: Eric — run every existing PROVEN file (MCC, AACC, Kennesaw) through the baseline parser, capture the rules it used into the `parserRules` DB as the active v1 baseline, and freeze each document's verified review screen as a GOLDEN snapshot. A golden E2E re-imports each proven doc and compares against its review screen (same std/sub-spec placement, same file-type classification, every card anchored/located). Guardrail: no Parser Train rule may go active if it regresses a golden test — the proven documents are the immovable floor beneath the self-improving loop. Build this FIRST (master the node).

## [2026-07-25] setup | CR-073 Phase 1 — Parser Train safety-net foundation shipped to dev + E2E-tested
Updated: [[change-requests/cr-073-self-improving-parser-agent]] (status → in-progress; Phase-1-shipped banner), change-requests/index.md.
Context: Built + tested on DEV the foundation ("master the node first"): (1) ParserRule model + parserrules collection + seed of 17 baseline rules (server/scripts/seed_baseline_parser_rules.mjs, seeded on dev); (2) parserContract §7 verifier + POST /api/imports/:id/contract-check (anchors gate + coverage/file-type/loss); (3) baseline captured from the three proven docs pulled from prod S3 → e2e/fixtures/golden/{aacc,kennesaw,mcc}.json; (4) golden regression E2E e2e/tests/73_parser_baseline_golden.spec.ts — GREEN on dev: AACC 20std/85spec/369 items, Kennesaw 10/35/209, MCC(@slow) 20/77/73-appendix/169, 0 un-anchored on all three (template placement exact, MCC structural, anchor gate enforced). Proven parser code untouched; rule engine + agent diagnose/refine + Parser Train SU UI are later phases. Dev commits 8e323be/d01686e/2b3bf3c; not on prod.

## [2026-07-26] setup | CR-073 Phase 2 — Parser Train SU training loop + rule engine shipped to dev
Updated: [[change-requests/cr-073-self-improving-parser-agent]] (Phase-2-shipped banner).
Context: Shipped to DEV (e9686ca) the human-in-the-loop + first realtime rule-engine lever. (1) Sandbox isolation: Submission.trainingRun flag; training runs excluded from every submission list; runs on a dedicated Parser Train Sandbox institution + PC. (2) parserTrainController + routes/parserTrain (SU-only): create sandbox run, diagnose (contract-check + auto-propose parserRules for critical gaps), set-rule (SU sets/activates a rule), approve-spec (activate the run's proposed rules — the approval gate), list runs. (3) Rule engine parserRuleEngine.resolveForceFormat — server consults ACTIVE per-institution rules at all 3 import-dispatch sites; DEFAULT-PRESERVING (explicit wins; no rule → unchanged) so MCC/AACC/Kennesaw + every real institution parse exactly as before; ai-status surfaces effective forceFormat. (4) Client: SU-only Parser Train page + nav (create → import as sandbox PC via per-request impersonation → diagnose → open Review/Compare → approve=activate). (5) E2E 74_parser_train: full loop on dev — diagnose 0 un-anchored, isolation (absent from /submissions), rule engine (forceFormat steer to self_study). Still open: the agent diagnose/refine LLM loop for full extraction rules, richer per-region rule application beyond forceFormat, prod rollout.

## [2026-07-26] setup | CR-073 Phase 3 — the ARCHITECTURE (runtime rule ENGINE + learning loop) shipped to dev + full E2E
Updated: [[change-requests/cr-073-self-improving-parser-agent]] (Phase-3 banner).
Context: Built + fully E2E-verified on DEV the self-improving CENTRE that Phases 1-2 scaffolded. (1) ai-service rule ENGINE (ai-service/app/rules/engine.py): at parse time reads parserrules from Mongo (3s timeout, try/except→[]) and RuleEngine.apply_post_pass reassigns ONLY items a rule's signature explicitly targets (sectionIdEquals/textContains/currentBucket) to extract.params std.spec + classification; wired in _run_pipeline after dispatch, before anchoring. Rules are DATA the engine EXECUTES now. GET /health/rules diagnostic. (2) DEFAULT-PRESERVING PROVEN: 15 baseline rules use structural signatures the post-pass doesn't evaluate → goldens byte-identical with engine live (AACC 85/Kennesaw 35, 0 un-anchored). (3) LEARNING LOOP (server/src/services/parserTrainAgent.ts, POST /:importId/auto-refine): re-parses under each candidate format, clears sandbox review state between candidates (else items accumulate across job-ids → stale unanchored), settles anchoring, scores vs §7 contract, writes winner as active institution rule; trajectory→Submission.parserTrainState, UI table, GET /refine-status. (4) GUARDRAIL: set-rule blocks global activation unless goldenChecked. (5) E2E GREEN: 73 goldens (default-preserving), 74 loop (85 specs/347 items anchored/isolation/forceFormat steer), 75 architecture (guardrail blocks global; engine moves narrative 1.a→9.j at parse time; loop learns 'auto' over self_study 64sp/MISSING), 76 screenshot walkthrough → PDF e2e/report/parser-train-e2e-report.pdf. Fixes: aiFormat-is-object, materialization race (poll buckets + settledContract), cross-job accumulation (clearReviewState), browser upload Content-Type, sandbox institution type=college. Commits f2fc888(engine)/8a8abce(loop)…8cdfcb6. NOT on prod. Remaining: LLM diagnose synthesising full extraction rules for novel shapes; prod rollout.

## [2026-07-26] setup | CR-073 items 1–13 COMPLETE + tested on Stevenson/Kennesaw/MCC (dev)
Updated: [[change-requests/cr-073-self-improving-parser-agent]].
Context: Completed the full gap list (1–13) the user itemised, tested by running the ORIGINAL Stevenson self-study (pulled from dev S3) as a brand-new document through the analyzer, plus Kennesaw + MCC (pulled from prod S3). Built: (#1) autonomous rule synthesis — parserTrainAgent.synthesizePlacementRules asks the matcher (new ai-service POST /ai/placement/recommend) where each UNPLACED tag belongs and writes a deterministic placement parserRule; diagnose proposes, the learning-loop Phase 2 activates + re-parses. (#2) engine apply_post_pass now routes unplaced job.tags + reclassifies file-types (appendix/syllabus/cv/matrix). (#3) per-region rules reconcile into ONE anchored review state (hybrid handling). (#4) train-mode Review banner (SU). (#5) notes+screenshots persisted on the run. (#6) approve-spec activates the SPECIFIC rule for that std.spec. (#7) automated golden guardrail (ParserTrainGuard marker gates global activation, stamped by the passing golden suite). (#8) dated per-run review page. E2E ALL GREEN on dev: #1/#3/#11 Stevenson (7 synth rules, unplaced 17→13, anchors OK), Kennesaw (9 rules, 10→2), MCC (0 rules, clean, all anchored); #9 MCC golden default-preserving (77 specs, 73 appendix, 169 anchored) + AACC/Kennesaw goldens exact; #10 file-type appendix stamp; #12 byte-for-byte isolation; #13 train banner + anchor-gate Compare; #5/#6/#7/#8 acceptance. Report: e2e/report/parser-train-e2e-report.pdf (14 suites, 9 screenshots). NOT on prod (per user). Docs pulled read-only from S3; sandbox rules cleaned to baseline after.

## [2026-07-27] update | CR-073 — parsing verified ON THE REVIEW PANEL (SU) + validated dev↔production
Updated: [[change-requests/cr-073-self-improving-parser-agent]]; SelfStudyEditor (review surface now a SUPERUSER surface for training runs).
Context: Correction — earlier the parse was qualified on the Self-Study Editor (files show only after approval), not the Review panel. Fix: the review panel (ReviewSurface→ReviewStep, SpecRail + ItemCardList) now renders for a superuser (canReviewSurface = isProgramCoordinator || isSuperuser) via /self-study/:id?view=review — Parser Train is SU-only. E2E 80 opens it as SU and captures the parsed result PER SECTION (Introduction / Standards parsed-narrative-per-spec / Compare card LOCATED in source / Files = Supporting Evidence rail) for AACC, Kennesaw, MCC, Stevenson. Each prod-available doc is VALIDATED against the PRODUCTION review panel — opened by impersonating that document's real PC (AACC=Nicole Williams, Kennesaw=Monica Nandan, MCC=Michelle Miller) — confirming the dev parse reproduces the validated prod outcome: AACC dev 20⊇prod 20 /85≈86, Kennesaw 10⊇10/35=35, MCC 20⊇20/77=77, intro present both. Stevenson (dev-only new doc): 28 stds/106 specs/106k intro/180 files. Report e2e/report/parser-train-review-panel-report.pdf (dev+prod per section). Docs pulled read-only from S3. Dev commits 1a6e983 (SU review surface), 1a638ca (test+report). NOT prod.

## [2026-07-27] fix | CR-073 — EXACT dev↔prod review-panel validation (no tolerance); SPLIT capability
Updated: [[change-requests/cr-073-self-improving-parser-agent]].
Context: Corrected a corner-cut — the dev↔prod validation used a spec-count tolerance + standards-superset, which hid a real gap. Redone with EXACT byStandard equality. Findings + fixes: (1) AACC dev=85 vs prod=86 — Std 15.b was BUNCHED into 15.a by the template walker (no letter marker); prod had it via a manual split. Built a SPLIT capability: engine apply_post_pass split pass (extract.split={boundary,std,spec} cuts the host narrative, emits the tail as the missing spec, fresh sectionIds so both re-anchor) + agent synthesizeSplitRules (detect a catalog spec with NO content whose prompt PREFIX appears mid-narrative in another bucket). The learning loop now autonomously un-bunches 15.b → dev 86 = prod 86 EXACT. (2) Split over-fired on MCC (invented 21.e/h/i from Field-Experience prompt MENTIONS in prose) → gated split synthesis to aiFormat==template only. (3) MCC test level was 'associate' but the prod submission is programLevel=bachelors → aligned. RESULT: EXACT byStandard match on all three prod docs — AACC 86=86, Kennesaw 35=35, MCC 77=77, introduction present on both, every Standard identical. Report e2e/report/parser-train-review-panel-report.pdf (dev+prod per section). Dev commits 2db5b13 (split), 8ee6bd0 (template gate + level fix). NOT prod.
