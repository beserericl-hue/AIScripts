...
- **S4.1 — Finish CR-018 evidence loop. ✅ DONE 2026-05-30.** New server `evidenceRecommendationsController` + `GET /api/submissions/:id/specs/:std/:spec/evidence-recommendations` (institutionId + submissionId + programLevel scoped per the cross-institution audit Gap 2 boundary). `cshseAiClient.EvidenceRecommendRequest` widened with `programLevel`. Reader UI: lazy `EvidenceRecommendations` panel mounted under each ReaderSpecRow via expandable "Show recommended evidence" toggle (fetch only fires once the reader asks). `cshse_evidence_{env}` Qdrant collection auto-bootstraps via `store.ensure_collection(...)` on first call. 7 server + 5 client tests. The n8n evidence workflow definitions live on the external n8n instance, not in this server tree; archiving is a discrete ops task on the n8n side.
...
- **S5.2 — Suggestions consolidation DOCX (CR-011, new). ✅ DONE 2026-05-30.** `services/suggestionsDocx.ts` (DOCX from `docx` package) pulls comments + reader override notes + AI suggestions, grouped by Standard → Specification. `GET /api/submissions/:id/compilation/suggestions-doc?mode=internal|pc_facing` — lead/admin only; server-side redaction in pc_facing (unrelayed dropped, pcLabel + relayedText for relayed, override notes suppressed). CompilationTab gains an export toolbar (mode radios + Generate button) that streams the .docx as a Blob and triggers download. Tests: 7 server integration (`tests/integration/suggestions-doc.test.ts` unzips the .docx and asserts redaction in `word/document.xml`) + 3 new client unit tests (toolbar render / mode toggle / Generate fires + disabled while exporting). CSHSE branded header/footer deferred.
- **S5.3 — Reader file attachments on comments (CR-021, new). ✅ DONE 2026-05-30 (server slice).** `Comment.attachments[]` (subdoc with s3Key + filename + mimeType + sizeBytes + uploadedBy + uploadedByName + uploadedAt) + `uploadCommentAttachment` / `downloadCommentAttachment` / `deleteCommentAttachment` controllers + multer memory storage with MIME allowlist (PDF / DOCX / TXT) + 25 MB cap. S3 prefix `reader-attachments/{commentId}/{ts}_{name}`. ACL follows relay state — `_canReadAttachment`: reader / lead_reader / admin / superuser always; PC only when `comment.relayed === true`. 11 integration tests (`tests/integration/comment-attachments.test.ts`, s3Service spied with `vi.spyOn`/`vi.restoreAllMocks`). Composer paperclip UI deferred until the reader-comment surface lands in a later sprint.
- **S5.4 — Portal direct messaging (CR-010, greenfield). ✅ DONE 2026-05-30.** `models/DirectMessage.ts` exports both `DirectMessageThread` (per submission; `participantIds[]`; optional `contextStandardCode`/`contextSpecCode`; `lastMessageAt`) and `DirectMessage` (per thread, append-only). `directMessageController` exports `listThreads`/`createThread`/`getThread`/`postMessage`. PC role is 403 on every endpoint via `_denyIfPC`; non-participant reader/lead → 403; admin/superuser bypass. Adding a PC as a participantId is rejected at create time (400). `createThread` is one-shot — creates the thread + posts the opening message. Routes: `GET|POST /api/submissions/:id/messages`, `GET|POST /api/messages/:threadId`. Client `features/reader/Messages/Messages.tsx` — pure `MessagesView` (sidebar threads + inline reader + composer) + container. Tests: 9 server integration + 7 client unit. Email + audit-log mirror deferred (DirectMessage is itself append-only via lack of update endpoint).
...
- **S6.1 — Partial-compliance checklist (CR-012). ✅ DONE 2026-05-30.** `SiteVisitChecklistItem` model (unique on sub/std/spec) + auto-population from `compilationController.setFinalScore` (score 0 → `non_compliant` row; 1 → `partial`; 2/3 removes auto unless verified; clear removes auto unless verified — verified rows survive a score-up so the visit team's note is sticky). New `checklistController` (list / manual-add / verify / delete / DOCX export) with role gates (PC 403; reader read-only; lead/admin write). New `services/siteVisitChecklistDocx.ts` produces a Spec / Inclusion-reason / Verified / Note table. Audit `checklist.item_verified` + `_unverified`. Client `features/siteVisit/Checklist/Checklist.tsx` (pure view + container) + `/site-visit/:submissionId/checklist` route. 11 server integration + 10 client view-unit tests (full client 279/279).
- **S6.2 — Itinerary builder (CR-013). ✅ DONE 2026-05-30.** `SiteVisit.agenda` subdoc widened with `location`/`attendees`/`specCodes`/`checklistItemIds`/`notes` (additive). New `itineraryController` (get/replace/exportDocx) with lead-OR-PC co-edit gate (admin bypass), audit `itinerary.updated` carries priorAgendaLen + newAgendaLen. DOCX export resolves linked checklist items inline (Standard X.y — reason — verified?). Client `features/siteVisit/Itinerary/Itinerary.tsx` (pure view + container) + `/site-visit/:submissionId/itinerary` route. 13 server integration + 10 client view-unit tests.
...
- **S7.2 — In-app bug reporter (CR-016). ✅ DONE 2026-05-30.** Server `BugReport` model + `POST /api/bug-reports` (any auth user; 400 on missing fields; server-side scrub for Bearer/JWT/AWS/secret tokens; console errors capped at last 10) + admin `GET /api/admin/bug-reports` + `PATCH /admin/bug-reports/:id` (status: new/triaged/resolved/dismissed + triageNote). Client `BugReporter` floating trigger + modal with auto-captured route + userAgent + buildSha (`VITE_BUILD_SHA`) + rolling console-error buffer (also captures `window.onerror` + `unhandledrejection`). Mounted in Layout. 7 server + 10 client tests. Auto-screenshot (html2canvas) deferred to a follow-on CR.
- **S7.3 — Admin audit-trail UI (CR-020). ✅ DONE 2026-05-30.** New `auditTrailController` (`listAuditEntries` + `exportAuditCsv`) admin/superuser only; filters by action (single or comma-list) / actorId / targetId / submissionId / since / until; newest-first; limit clamped to [1, 500] for paged reads + [1, 10000] for CSV. Append-only invariant lives at the model layer. Client `features/admin/AuditTrail/AuditTrail.tsx` (pure view + container) mounted at `/admin/audit-trail`. 5 server integration + 6 client unit tests.
- **S7.4 — E2E coverage expansion. ✅ DONE 2026-05-30 (smoke layer).** `e2e/tests/19_sprint7_smoke.spec.ts` — API-level Playwright spec exercising every new Sprint 7 endpoint against the deployed env via the existing `/api/test/seed` (CR-034) fixture: board decision round-trip + audit-log + CSV export + bug-reporter submit/list + checklist + itinerary + evidence-recommendations. Skips gracefully without `E2E_SEED_TOKEN`. Matches the lightweight pattern of `00_health.spec.ts` (no browser navigation); heavier UI-driven E2E (wizard flows) lives in the 02..17 specs. Deeper per-feature UI E2Es deferred — the API smokes catch endpoint regressions; full reader→lead→board UI walkthrough lands when product asks for it.
