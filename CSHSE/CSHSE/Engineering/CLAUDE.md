...
LLM-maintained wiki for the CSHSE Accreditation Self-Study Portal.

## Top-level folders

Content is organised by domain at the top level. Each folder is a distinct concern:

- `Engineering/` — system architecture, code reviews, sprint plans, security audits, concept pages for the codebase. The bulk of the vault. This file (`CLAUDE.md`) lives here and governs the whole vault.
- `Marketing/` — sales copy, video scripts, film plans, distribution plans. Customer-facing artefacts.

When adding a top-level folder, add it to this list and update `Engineering/index.md` (or create a sibling index file in the new folder).
...
- `marketing` — dated, forward-looking marketing artefact (sales copy + video scripts + film plans + distribution plans for a launch or campaign). Filename includes the date; supersede with a new dated plan when the product or messaging changes.
...
`plan` and `marketing` pages additionally include `plan_date: YYYY-MM-DD` and `status: <proposed | ready-to-produce | in-progress | superseded>`.
