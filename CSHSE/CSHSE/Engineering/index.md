...
- [[email-deliverability]] — outbound mail: SendGrid-primary (Postal fallback), fixed branded From, trust footer + Tigard postal address, the Spamhaus/Railway-IP story, and how to verify delivery via the SendGrid Activity API. **Added 2026-06-16.**
...
- [[monica-review-walkthrough-2026-06-26]] — first real-user pass over the Review panel (Monica Nandan / KSU, on prod). Catalogues **parser data-loss** (specs reduced to their appendix-pointer line; missing links + flattened lists; inconsistent tables — image49/50/45/65) and the dominant **UI-noise** complaint (show only mode-relevant controls), plus the red-dot-vs-0.92-confidence collision and clickable-counts request. Evidenced by the Otter transcript + 84 screen-share screenshots. → [[review-panel-ux-plan-2026-06-26]].
...
- [[review-panel-ux-plan-2026-06-26]] — focused fix plan from Monica's walkthrough ([[monica-review-walkthrough-2026-06-26]]): **P1** parser data-loss audit+fix, **P2** mode-aware de-noise of Review, **P3** full-section compare highlight, **P4** clickable count filters + indicator legend, **P5** inline appendix upload at center-pane placeholders + self-study leak. Complements (does not supersede) [[sprint-plan-2026-05-31]].
- [[importer-review-sprint-2026-06-26]] — **execution sprint** for the above, scoped to CRs **cr-061…072**. **Sprint 1 = importer fidelity (P0, blocking)** — "write everything until the next break"; no dropped content/links/images/tables; audit → fix → KSU regression → re-import dev+prod. **Sprint 2 = Review/Self-Study de-noise + high-value workflow** (mode-aware chrome, AI-eval modal, approve rename, full-section highlight, clickable counts, dot legend, inline appendix upload, self-study munge fix). Sprint 3 (opt) = compare sync-scroll. Each CR: made → verified → tested → demoed to Monica.
