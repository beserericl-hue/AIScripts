...
// CR-073 — Parser Train (superuser-only) surface.
const ParserTrainIcon = () => (
  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function Layout() {
  ...
  // CR-073 — Parser Train, the self-improving parser training surface. Superuser
  // ONLY, and only when not impersonating (it acts on a sandbox institution and
  // activates parser rules — a true privileged action).
  if (isCurrentlySuperuser && !impersonation.isImpersonating) {
    navigation.push({ name: 'Parser Train', href: '/parser-train', icon: ParserTrainIcon, tourStep: 'parser-train' });
  }
