...
//   2. "(Re)start the tour" — shown only on /dashboard; label flips on
//                             tourStatus.welcome.
//
...
export function HelpMenu({ className }: HelpMenuProps): JSX.Element {
  ...
  const { tourStatus, startTour } = useTour();
  ...
  const onTour = () => {
    startTour('welcome', { autoStarted: false });
    setOpen(false);
  };

  const isOnHome = location.pathname === HOME_ROUTE;
  const tourLabel = tourStatus.welcome ? t('help.menu.restartTour') : t('help.menu.startTour');
