...
interface ReportSpec {
  specCode: string;
  specTitle: string;
  // The official standard/specification language for this row — shown on demand
                  ...
                  <div className="lg:sticky lg:top-4 lg:w-80 lg:shrink-0">
                    <div data-testid={`rr-check-${r.code}-${sp.specCode}`} className="rounded-lg border-2 border-teal-300 bg-white shadow-sm">
                      ...
                      <div className="space-y-3 p-3">
                        {/* The official standard/specification LANGUAGE for this
                            row, revealed on demand — readers asked to see the
                            actual standard text next to the narrative (the paper
                            reader form prints it), not just the short title. */}
                        {sp.specText && (
                          <details data-testid={`rr-standard-text-${r.code}-${sp.specCode}`} className="rounded-md border border-slate-200 bg-slate-50">
                            <summary className="flex cursor-pointer list-none items-center gap-1.5 rounded-md px-2.5 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-100">
                              <BookOpen className="h-3.5 w-3.5" />
                              {r.code === 'introduction' ? 'View the reader-form criteria' : `View Standard ${r.code}.${sp.specCode}`}
                              <span className="ml-auto font-normal text-slate-500">view</span>
                            </summary>
                            <div className="max-h-64 overflow-y-auto border-t border-slate-200 px-2.5 py-2 text-xs leading-relaxed text-slate-700">
                              {sp.specText}
                            </div>
                          </details>
                        )}
