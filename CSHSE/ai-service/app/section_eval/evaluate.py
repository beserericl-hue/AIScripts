...
def _build_prompt(
    *,
    ...
    return (
        ...
        "SCORING CALIBRATION — judge like a fair human reader:\n"
        "1. Requirements vs recommendations: items phrased 'recommended' or 'strongly "
        "recommended' are NOT required. Never fail or downgrade a spec because a RECOMMENDATION "
        "is unmet — if the actual requirement is met, mark the criterion met and note the "
        "recommendation as an optional suggestion.\n"
        "2. Substance over enumeration: mark a criterion met when its CORE requirement is "
        "substantively addressed; do NOT fail for lacking explicit separate mention of every "
        "enumerated sub-item when the narrative + evidence reasonably cover the intent (e.g. "
        "'faculty make all curriculum decisions' covers setting policy, content, implementation "
        "and evaluation).\n"
        "3. Documents/matrices/forms: when the criterion asks that a document, matrix, or form be "
        "PROVIDED, its EXISTENCE in the file library satisfies it — do not fail because a table's "
        "exact cell contents can't be read from the extracted text.\n\n"
